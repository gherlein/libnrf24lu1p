...
#define CRDEBUG(format, args...) nrf24radio_log(4, format, ##args)
#define CRINFO(format, args...) nrf24radio_log(3, format, ##args)
#define CRWARN(format, args...) nrf24radio_log(2, format, ##args)
#define CRERROR(format, args...) nrf24radio_log(1, format, ##args)
#define CRFATAL(format, args...) nrf24radio_log(0, format, ##args)
...
const char *nrf24radio_get_errorstr(void) {
...
static int set_nrf24radio_error(int error_code) {
...
static void nrf24radio_exit(char *format, ...) {
    va_list args;
    char *newfmt;
    char *prefix="ERROR: ";
 ...
 * ignored.
 */
int nrf24radio_set_channel(nrf24radio_device_t *prd, uint16_t channel) {
    if(channel > 126)
        return set_nrf24radio_error(CR_ERR_BADCHANNEL);

    CRDEBUG("Setting channel to %02x", channel);
    return nrf24radio_send_config(prd, CONF_SET_RADIO_CHANNEL, channel, 0, NULL, 0);
}
 ...
 */
int nrf24radio_set_address(nrf24radio_device_t *prd, nrf24radio_address address) {
    CRDEBUG("Setting address to %02x %02x %02x %02x %02x", address[0],
            address[1], address[2], address[3], address[4]);

    return nrf24radio_send_config(prd, CONF_SET_RADIO_ADDRESS, 0, 0,
                              (uint8_t*)address, 5);
}
 ...
 * DATA_RATE_2MBPS
 */
int nrf24radio_set_data_rate(nrf24radio_device_t *prd, uint16_t data_rate) {
    if(data_rate > DATA_RATE_2MBPS)
        return set_nrf24radio_error(CR_ERR_BADDATARATE);

    CRDEBUG("Setting data rate to %02x", data_rate);
    return nrf24radio_send_config(prd, CONF_SET_DATA_RATE, data_rate, 0, NULL, 0);
}
 ...
 */
int nrf24radio_set_power(nrf24radio_device_t *prd, uint16_t power) {
    if(power > POWER_0DBM)
        return set_nrf24radio_error(CR_ERR_BADPOWER);

    CRDEBUG("Setting power to %02x", power);
    return nrf24radio_send_config(prd, CONF_SET_RADIO_POWER, power, 0, NULL, 0);
}

/* Set ack enable */
int nrf24radio_set_ack_enable(nrf24radio_device_t *prd, uint16_t enable_status) {
    CRDEBUG("%sabling auto-ack", enable_status ? "en" : "dis");
    return nrf24radio_send_config(prd, CONF_ACK_ENABLE, enable_status, 0, NULL, 0);
}
 ...
 * value must be 0-15
 */
int nrf24radio_set_arc(nrf24radio_device_t *prd, uint16_t arc) {
    if(arc > 15)
        return set_nrf24radio_error(CR_ERR_BADARC);

    return nrf24radio_send_config(prd, CONF_SET_RADIO_ARC, arc, 0, NULL, 0);
}
 ...
 * ignored
 */
int nrf24radio_set_ard_time(nrf24radio_device_t *prd, int us) {
    uint16_t ard_time;

    if(us > 4000)
        return set_nrf24radio_error(CR_ERR_BADARDTIME);

    ard_time = (us / 150) - 1;

    CRDEBUG("Setting ard time %02x", ard_time);
    return nrf24radio_send_config(prd, CONF_SET_RADIO_ARD, ard_time, 0, NULL, 0);
}
 ...
 * by time (nrf24radio_set_ard_time) or by pack payload length.  If ack
 * payload length is set, then ARD will be altered even if the
 * data rate is changed.
 *
 * size can be 0-32 bytes
 */
int nrf24radio_set_ard_bytes(nrf24radio_device_t *prd, uint16_t bytes) {
    if(bytes > 32)
        return set_nrf24radio_error(CR_ERR_BADARDPKT);

    CRDEBUG("Setting ard bytes to %02x", bytes);
    return nrf24radio_send_config(prd, CONF_SET_RADIO_ARD,
                              bytes | 0x80, 0, NULL, 0);
}
...
static int nrf24radio_xfer_packet(nrf24radio_device_t *prd, unsigned char endpoint,
...
int nrf24radio_read_packet(nrf24radio_device_t *prd, unsigned char *buffer,
                       int len, int timeout) {
    return nrf24radio_xfer_packet(prd, 0x81, buffer, len, timeout);
}

/* write a packet (only valid in PTX mode) */
int nrf24radio_write_packet(nrf24radio_device_t *prd, unsigned char *buffer,
                        int len, int timeout) {
    return nrf24radio_xfer_packet(prd, 0x01, buffer, len, timeout);
}

int nrf24radio_close(nrf24radio_device_t *prd) {
    libusb_device_handle *handle = (libusb_device_handle*)prd->pusb_handle;
